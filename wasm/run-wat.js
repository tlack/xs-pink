...
function main(){
	...
	emit(process.argv,'argv');
	var fn=process.argv[2];
	if(!fn) throw 'run as node run-wat.js [filename.wat] [func|start]';
	emit(fn,'compiling');
	var c=getSrc(fn),b=wast.WebAssemblyText.encode(c);
	emit(c);
	var sys={emit:emit,mem:Mem};
	var wm=new WebAssembly.Module(b);
	var winst=new WebAssembly.Instance(wm,{sys:sys});
	//emit(winst);
	func=process.argv[3] || 'start';
	winst.exports[func](process.argv.slice(3));
	dump();
}
main();
